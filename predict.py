...
from pipeline import StableDiffusionXLReferencePipeline
...
class Predictor(BasePredictor):
    ...
    def setup(self, weights: Optional[Path] = None):
        ...
        print("Loading SDXL reference pipeline")
        self.ref_pipe = StableDiffusionXLReferencePipeline.from_pretrained(
            SDXL_MODEL_CACHE,
        ...
        if weights or os.path.exists("./trained-model"):
            self.load_trained_weights(weights, self.ref_pipe)

        self.ref_pipe.to("cuda")
    ...
    def predict(
        ...
        image: Path = Input(
            description="Reference image to make variations of",
            default=None,
        ),
        width: int = Input(
        ...
        if image is None:
            raise Exception("Image is required for prediction.")
        ...
        if replicate_weights:
            self.load_trained_weights(replicate_weights, self.ref_pipe)

        # OOMs can leave vae in bad state
        if self.ref_pipe.vae.dtype == torch.float32:
            self.ref_pipe.vae.to(dtype=torch.float16)
        ...
        common_args = {
            "ref_image": self.load_image(image),
            "width": width,
            "height": height,
